invDasymetry_NoPar <- function(clmn = "ZONE", value = "Europe", aggr = NULL, 
    ...
    for (i in 1:nrow(plots.tf)){
      ...
      if(!is.null(aggr)){
        rw <- c(treeCovers * plots.tf$AGB_T_HA[i],
        ...
        FFAGB <- rbind(FFAGB, rw)
      }
      else{
        rw <- c(treeCovers * plots.tf$AGB_T_HA[i],plots.tf$AGB_T_HA[i],
